...
const useRoomSocket = (setParticipants, consume, createRecvTransport) => {
  useEffect(() => {
    ...
    socket.on("toggle-hand", ({ socketId, toggleHand }) => {
      ...
      setParticipants((prev) => prev.map((p) => (p.socketId === socketId ? { ...p, toggleHand } : p)));
    });
